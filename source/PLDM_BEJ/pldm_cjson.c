...
void pldm_cjson_printf_root2(pldm_cjson_t *root)
{
    pldm_cjson_t *tmp = root;
    while (tmp) {
        if (tmp->name) LOG("seq : 0x%02x, fmt : 0x%02x, len : %d, name : %s : ", tmp->sflv.seq, tmp->sflv.fmt, tmp->sflv.len, tmp->name);
...
u16 pldm_cjson_cal_len_to_root2(pldm_cjson_t *root, u8 op_type)
{
    ...
    while (tmp) {
        ...
        if (fmt != BEJ_ENUM) tmp->sflv.len += pldm_cjson_cal_len_to_root2(tmp->child, op_type);
        if (op_type != HEAD) {
            if (fmt != BEJ_SET && fmt != BEJ_ARRAY) {
...
static void pldm_cjson_fill_comm_field_in_schema(pldm_cjson_t *root, u8 is_collection, u32 id, char *type, u8 resource_identify)
{
    ...
    for (u8 i = 0; i < cnt; i++) {
        if (i < 4) {
            ...
            f_sflv.seq = 0 << 1;
            is_find = NULL;
            pldm_cjson_find_name(root, key[i]);
            if (is_find) {
                sflv.fmt = BEJ_ENUM << 4;
                pldm_cjson_add_item_to_obj(is_find, &sflv, "HealthRollup", val[i], 2);
            } else {
                sflv.fmt = BEJ_SET << 4;
...
static pldm_cjson_schema_fmt_t *pldm_cjson_create_schema_recursion(pldm_cjson_t *obj, pldm_cjson_schema_fmt_t *fmt)
{
    ...
    if (buf[0].fmt == BEJ_SET || buf[0].fmt == BEJ_ARRAY || buf[0].fmt == BEJ_ENUM) {
        cnt = buf[0].child_cnt;
        // LOG("name : %s\n", tmp->name);
        if (buf[0].fmt == BEJ_ENUM)
    ...
    if (!cnt && BEJ_SET != sflv.fmt >> 4 && BEJ_ARRAY != sflv.fmt >> 4 && BEJ_ENUM != sflv.fmt >> 4) {
...
typedef struct {
    u8 child_cnt;
    pldm_cjson_t *node;
} pldm_cjson_create_schema_recursion_1_t;
...
static void pldm_cjson_create_schema(pldm_cjson_t *obj, pldm_cjson_schema_fmt_t *fmt, u8 len)
{
    ...
    pldm_cjson_schema_fmt_t *buf = fmt;
    pldm_cjson_t *tmp = obj;
    pldm_cjson_create_schema_recursion_1_t schemas[8];
    int idx = -1;
    schemas[0].node = tmp;
...
pldm_cjson_t *pldm_cjson_create_event_schema(u32 resource_id, u8 link_state)
{
    ...
    // pldm_cjson_create_schema_recursion(root, fmt, sizeof(fmt) / sizeof(pldm_cjson_schema_fmt_t));
        ...
        {0, BEJ_SET, 1, "", ""},
            {0, BEJ_SET, 14, "Port", ""},
                ...
                {0, BEJ_SET, 3, "Ethernet", ""},
                    {0, BEJ_ENUM, 3, "FlowControlConfiguration", (char [3]){0x01, 0x01, 0x00}},
                        {0, BEJ_STR, 0, "TX", ""},
                        {0, BEJ_STR, 0, "RX", ""},
                        {0, BEJ_STR, 0, "TX_RX", ""},
                    {0, BEJ_ENUM, 3, "FlowControlStatus", (char [3]){0x01, 0x01, 0x00}},
                        {0, BEJ_STR, 0, "TX", ""},
                        {0, BEJ_STR, 0, "RX", ""},
                        {0, BEJ_STR, 0, "TX_RX", ""},
                    {0, BEJ_ARRAY, 1, "SupportedEthernetCapabilities", ""},
                        {0, BEJ_ENUM, 2, "", (char [3]){0x01, 0x01, 0x00}},
                ...
                {0, BEJ_ARRAY, 1, "LinkConfiguration", ""},
                    {0, BEJ_SET, 3, "", ""},
                        ...
                        {0, BEJ_ARRAY, 2, "CapableLinkSpeedGbps", (char [3]){0x01, 0x20, 0x00}},
                            {0, BEJ_REAL, 0, "", "Gbs"},
                            {0, BEJ_REAL, 0, "", "Gbs"},
                {0, BEJ_ENUM, 4, "LinkNetworkTechnology", (char [3]){0x01, 'U', 0x00}},
                    {0, BEJ_STR, 0, "Ethernet", ""},
                    {0, BEJ_STR, 0, "InfiniBand", ""},
                    {0, BEJ_STR, 0, "FibreChannel", ""},
                    {0, BEJ_STR, 0, "GenZ", ""},
                {0, BEJ_ENUM, 2, "LinkState", (char [3]){0x01, 'U', 0x00}},
                    {0, BEJ_STR, 0, "Enabled", ""},
                    {0, BEJ_STR, 0, "Disabled", ""},
                {0, BEJ_ENUM, 5, "LinkStatus", (char [3]){0x01, 0x01, 0x00}},
                    {0, BEJ_STR, 0, "LinkUp", ""},
                ...
                {0, BEJ_SET, 1, "Status", ""},
                    {0, BEJ_ENUM, 3, "State", (char [3]){0x01, 0x01, 0x00}},
                        {0, BEJ_STR, 0, "Absent", ""},
                        {0, BEJ_STR, 0, "Enabled", ""},
                        {0, BEJ_STR, 0, "Disabled", ""},
    };
                ...
                {0, BEJ_ARRAY, 1, "Controllers", ""},
                    {0, BEJ_SET, 4, "", ""},
                        ...
                        {0, BEJ_SET, 4, "PCIeInterface", ""},
                            ...
                            {0, BEJ_ENUM, 4, "MaxPCIeType", (char [3]){0x01, 0x01, 0x00}},  /* Maximum Link Speed? */
                                {0, BEJ_STR, 0, "Gen1", ""},
                                {0, BEJ_STR, 0, "Gen2", ""},
                                {0, BEJ_STR, 0, "Gen3", ""},
                                {0, BEJ_STR, 0, "Gen4", ""},
                            {0, BEJ_ENUM, 4, "PCIeType", (char [3]){0x01, 0x01, 0x00}},
                                {0, BEJ_STR, 0, "Gen1", ""},
                                {0, BEJ_STR, 0, "Gen2", ""},
                                {0, BEJ_STR, 0, "Gen3", ""},
                                {0, BEJ_STR, 0, "Gen4", ""},
    ...
    pldm_cjson_schema_fmt_t fmt[] = {
        ...
        {0, BEJ_SET, 1, "", ""},
            {0, BEJ_SET, 14, "NetworkDeviceFunction", ""},
                ...
                {1, BEJ_INT, 0, "@odata.count", (char [2]){MAX_LAN_NUM, 0x00}},
                {0, BEJ_ENUM, 5, "BootMode", (char [3]){0x01, 0x01, 0x00}},
                    {0, BEJ_STR, 0, "Disabled", ""},
                    {0, BEJ_STR, 0, "PXE", ""},
                    {0, BEJ_STR, 0, "iSCSI", ""},
                    {0, BEJ_STR, 0, "FibreChannel", ""},
                ...
                {0, BEJ_ARRAY, 1, "NetDevFuncCapabilities", ""},
                    {0, BEJ_ENUM, 5, "", (char [3]){0x01, 0x01, 0x00}},
                        {0, BEJ_STR, 0, "Disabled", ""},
                        {0, BEJ_STR, 0, "Ethernet", ""},
                        {0, BEJ_STR, 0, "FibreChannel", ""},
                        {0, BEJ_STR, 0, "iSCSI", ""},
                        {0, BEJ_STR, 0, "FibreChannelOverEthernet", ""},
                {0, BEJ_ENUM, 5, "NetDevFuncType", (char [3]){0x01, 0x01, 0x00}},
                    {0, BEJ_STR, 0, "Disabled", ""},
                    {0, BEJ_STR, 0, "Ethernet", ""},
                    {0, BEJ_STR, 0, "FibreChannel", ""},
                    {0, BEJ_STR, 0, "iSCSI", ""},
                ...
                {1, BEJ_SET, 2, "@Redfish.Settings", ""},
                    ...
                    {1, BEJ_ARRAY, 1, "SupportedApplyTimes", ""},
                        {1, BEJ_ENUM, 4, "", (char [3]){0x01, 0x03, 0x00}},
                            {1, BEJ_STR, 0, "AtMaintenanceWindowStart", ""},
                            {1, BEJ_STR, 0, "Immediate", ""},
                            {1, BEJ_STR, 0, "InMaintenanceWindowOnReset", ""},
                            {1, BEJ_STR, 0, "OnReset", ""},
                {0, BEJ_STR, 0, "Id", "Resource Offset"},
                {0, BEJ_SET, 2, "Links", ""},
    ...
    pldm_cjson_schema_fmt_t fmt[] = {
        ...
        {0, BEJ_SET, 1, "", ""},
            {0, BEJ_SET, 13, "PCIeDevice", ""},
                ...
                {0, BEJ_ENUM, 3, "DeviceType", (char [3]){0x01, 0x01, 0x00}},  /* Config space of function 0 - Header type register */
                ...
                {0, BEJ_SET, 4, "PCIeInterface", ""},
                    ...
                    {0, BEJ_ENUM, 5, "MaxPCIeType", (char [3]){0x01, 0x01, 0x00}}, /* Maximum Link Speed */
                        {0, BEJ_STR, 0, "Gen1", ""},
                        {0, BEJ_STR, 0, "Gen2", ""},
                        {0, BEJ_STR, 0, "Gen3", ""},
                        {0, BEJ_STR, 0, "Gen4", ""},
                        {0, BEJ_STR, 0, "Gen5", ""},
                    {0, BEJ_ENUM, 5, "PCIeType", (char [3]){0x01, 0x01, 0x00}},    /* Current Link Speed */
                        {0, BEJ_STR, 0, "Gen1", ""},
                        {0, BEJ_STR, 0, "Gen2", ""},
                        {0, BEJ_STR, 0, "Gen3", ""},
                        {0, BEJ_STR, 0, "Gen4", ""},
    ...
    pldm_cjson_schema_fmt_t fmt[] = {
        ...
        {0, BEJ_SET, 1, "", ""},
            {0, BEJ_SET, 11, "PCIeFunction", ""},
                ...
                {0, BEJ_ENUM, 1, "DeviceClass", (char [3]){0x01, 'U', 0x00}},
                    {0, BEJ_STR, 0, "NetworkController", ""},
                {0, BEJ_STR, 0, "DeviceId", ""},
                {0, BEJ_ENUM, 2, "FunctionType", (char [3]){0x01, 'U', 0x00}},
    ...
    pldm_cjson_schema_fmt_t fmt[] = {
        ...
        {0, BEJ_SET, 1, "", ""},
            {0, BEJ_SET, 12, "EthernetInterface", ""},
                ...
                {0, BEJ_SET, 1, "Status", ""},
                    {0, BEJ_ENUM, 5, "State", (char [3]){0x01, 0x03, 0x00}},
                        {0, BEJ_STR, 0, "StandbyOffline", ""},
                        {0, BEJ_STR, 0, "Starting", ""},
                        {0, BEJ_STR, 0, "Updating", ""},
                        {0, BEJ_STR, 0, "Enabled", ""},
...
static pldm_redfish_dictionary_entry_t *pldm_cjson_dict_fill_sf(u8 *dict, pldm_redfish_dictionary_entry_t *entry, u16 entry_cnt, pldm_cjson_t *root, u8 name_idx)
{
    ...
    LOG("fmt err: %#x, %s, %d", root->sflv.fmt, &(root->name[name_idx]), cm_strlen(&(root->name[name_idx])));
    return entry;
}
...
void pldm_cjson_cal_sf_to_root_op(pldm_cjson_t *root, u8 *anno_dict, u8 *dict, pldm_redfish_dictionary_entry_t *entry, u16 entry_cnt, pldm_redfish_dictionary_entry_t *anno_entry, u16 anno_entry_cnt)
{
    ...
    while (tmp) {
        ...
        if (fmt == BEJ_SET || fmt == BEJ_ARRAY || fmt == BEJ_ENUM) {
...
u32 g_resource_id[] = {
    ...
    PLDM_BASE_NETWORK_DEV_FUNC_RESOURCE_ID + 0,
    PLDM_BASE_NETWORK_DEV_FUNC_RESOURCE_ID + 1,
    PLDM_BASE_NETWORK_DEV_FUNC_RESOURCE_ID + 2,
    PLDM_BASE_NETWORK_DEV_FUNC_RESOURCE_ID + 3,
    PLDM_BASE_PORT_RESOURCE_ID + 0,
    PLDM_BASE_PORT_RESOURCE_ID + 1,
    PLDM_BASE_PORT_RESOURCE_ID + 2,
    PLDM_BASE_PORT_RESOURCE_ID + 3,
    PLDM_BASE_PCIE_FUNC_RESOURCE_ID + 0,
    PLDM_BASE_PCIE_FUNC_RESOURCE_ID + 1,
    PLDM_BASE_PCIE_FUNC_RESOURCE_ID + 2,
    PLDM_BASE_PCIE_FUNC_RESOURCE_ID + 3,
    PLDM_BASE_ETH_INTERFACE_RESOURCE_ID + 0,
    PLDM_BASE_ETH_INTERFACE_RESOURCE_ID + 1,
    PLDM_BASE_ETH_INTERFACE_RESOURCE_ID + 2,
    PLDM_BASE_ETH_INTERFACE_RESOURCE_ID + 3,
...
void swap_node(pldm_cjson_t *node1, pldm_cjson_t *node2)
{
    pldm_cjson_t tmp;
    ...
    tmp.sflv.val = node1->sflv.val;

    node1->child = node2->child;
    // node1->next = node2->next;
    node1->name = node2->name;
    node1->sflv = node2->sflv;

    node1->sflv.val = node2->sflv.val;

    node2->child = tmp.child;
    // node2->next = tmp->next;
    node2->name = tmp.name;
    node2->sflv = tmp.sflv;

    node2->sflv.val = tmp.sflv.val;
}

void pldm_cjson_list_sort(pldm_cjson_t *root)
{
    pldm_cjson_t *tmp = root;
    while (tmp) {
        if (tmp->child) {
            pldm_cjson_t *p = tmp->child;
            pldm_cjson_t *q = p;
            for (; p; p = p->next) {
                for (q = p->next; q; q = q->next) {
                    if ((p->sflv.seq >> 1) > (q->sflv.seq >> 1)) {
                        swap_node(p, q);
                    }
                }
            }
        }
        pldm_cjson_list_sort(tmp->child);
        tmp = tmp->next;
    }
}
...
static void pldm_cjson_schema_test(void)
{
    ...
    for (u8 i = 0; i < sizeof(g_schemas) / sizeof(schema_create); i++) {
        ...
        max_cnt = MAX(max_cnt, pldm_cjson_get_used_space());
        LOG("\nused space : %d, max_space : %d", pldm_cjson_get_used_space(), MY_CJSON_POLL_SIZE);
        LOG("resource id : %d, len : %d, cnt : %d", g_resource_id[i], end_ptr - bej_data, i);
        pldm_cjson_update_etag(root);
        pldm_cjson_pool_init();
    }
    LOG("max cnt : %d", ALIGN(max_cnt * 2, 512));
}
...
static void pldm_cjson_bej_test(void)
{
    ...
    for (u8 i = 0; i < sizeof(g_resource_id) / sizeof(u32); i++) {
        // if (g_resource_id[i] != g_resource_id[1]) continue;
        // pldm_cjson_pool_init();
        u8 ret = pldm_redfish_get_dict_data(g_resource_id[i], SCHEMACLASS_MAJOR, \
        g_needed_dict, pldm_redfish_get_dict_len(g_resource_id[i]));
        if (ret == false) return;
        ...
        bej_root = pldm_bej_decode(buf, ptr - buf, annc_dict, dict, bej_root, 1);
        if (bej_root) {
            LOG("decode cnt : %d\n", i);
...
extern char *bej_name[32];
extern u8 bej_name_idx;
extern void pldm_bej_fill_name(pldm_cjson_t *schema_root, pldm_cjson_t *bej_root);
...
void pldm_cjson_redfish_op_test(void)
{
    ...
    u8 op_type = UPDATE;
    u32 resource_id = PLDM_BASE_PORT_RESOURCE_ID;
    ...
    g_resourse_bej[resourse_indenty + offset + 6].is_bej = 0;
    LOG("resourse_indenty : %d", resourse_indenty);

    if (g_resourse_bej[resourse_indenty + offset].is_bej)
        root = pldm_bej_decode(g_resourse_bej[resourse_indenty + offset + 6].data, g_resourse_bej[resourse_indenty + offset].len, anno_dict, dict, root, 1);
    else root = g_schemas[resourse_indenty + offset + 6](resource_id);

    bej_buf.len = sizeof(bejencoding_t);
    u8 test_buf[] = {};
    ...
    if (bej_buf.len && op_type != READ && op_type != HEAD && op_type != ACTION) {
        bej_root = pldm_bej_decode(&(bej_buf.data[sizeof(bejencoding_t)]), bej_buf.len - sizeof(bejencoding_t), anno_dict, dict, bej_root, 0);
    ...
    pldm_cjson_t *match_node = pldm_bej_get_match_node();
    if (!match_node) LOG("pldm_bej_get_match_node err!");
    pldm_cjson_cal_len_to_root2(bej_root, op_type);
    // pldm_cjson_printf_root2(bej_root);
    int state = -1;
...
static void pldm_cjson_oem_dict_test(void)
{
    ...
    u8 dict[1024];
    u8 anno_dict[1024];
    root = pldm_cjson_create_networkdevicefunctioncollection_schema(resource_id);
